...
def main():
    ...
    train_dataset_event = eval('dataset.'+cfg.DATASET.DATASET)(
        ...
        transforms_event=transforms.Compose([
            ...
            transforms.ToTensor(),
            FillEventBlack(),
            transforms.RandomApply([RandomEventNoise()], p=0.8),
            transforms.RandomApply([RandomEventPatchNoise(brighten=True), RandomEventPatchNoise(brighten=False)], p=0.8),
    ...
    train_loader_event = torch.utils.data.DataLoader(
        train_dataset_event,
    ...
    for epoch in range(begin_epoch, cfg.TRAIN.END_EPOCH):
        ...
        # train for one epoch
        train(cfg, train_loader_event, model_event, model_hrnet_encoder_event,
            heatmap_loss_event,
            optimizer_event, epoch,
            final_output_dir, tb_log_dir, writer_dict)


        # evaluate on validation set
        perf_indicator_event = validate_certifier(
            cfg, valid_loader_event, valid_dataset_event,
            model_event,
            model_hrnet_encoder_event,
            heatmap_loss_event,
