...
class BPnP(torch.autograd.Function):
    ...
    """
    @staticmethod
    def forward(ctx, pts2d, pts3d, K, keypoint_scores=None, ini_pose=None):
        bs = pts2d.size(0)
        n = pts2d.size(1)
        device = pts2d.device
        ...
        for i in range(bs):
            ...
            if ini_pose is None:
               # _, rvec0, T0, _ = cv.solvePnPRansac(objectPoints=pts3d_np, imagePoints=pts2d_i_np, cameraMatrix=K_np, distCoeffs=None, flags=cv.SOLVEPNP_ITERATIVE, confidence=0.9999 ,reprojectionError=3)
                _, rvec0, T0, _ = cv.solvePnPRansac(objectPoints=pts3d_np, imagePoints=pts2d_i_np, cameraMatrix=K_np, distCoeffs=None, flags=cv.SOLVEPNP_EPNP, iterationsCount=10000, reprojectionError=10.0)
            else:
                rvec0 = np.array(ini_pose[i, 0:3].cpu().view(3, 1))
                T0 = np.array(ini_pose[i, 3:6].cpu().view(3, 1))
            _, rvec, T, _ = cv.solvePnPRansac(objectPoints=pts3d_np, imagePoints=pts2d_i_np, cameraMatrix=K_np, distCoeffs=None, flags=cv.SOLVEPNP_EPNP, iterationsCount=10000, reprojectionError=10.0, useExtrinsicGuess=True, rvec=rvec0, tvec=T0)
    ...
    def backward(ctx, grad_output):
        ...
        for i in range(bs):
            ...
            for j in range(m):
                ...
                R = axis_angle_to_rotation_matrix(P_6d_flat[0:m-3].view(1,3))
...
def batch_project(P, pts3d, K, angle_axis=True, clamp_to_resolution=None):
    n = pts3d.size(0)
    bs = P.size(0)
    device = P.device
    pts3d_h = torch.cat((pts3d, torch.ones(n, 1, device=device)), dim=-1)
    if angle_axis:
        R_out = axis_angle_to_rotation_matrix(P[:, 0:3].view(bs, 3))
    ...
    if clamp_to_resolution is not None:
        batch_size = pts2d_pro.shape[0]
        pts2d_pro = pts2d_pro.reshape((-1,2))
        coords_projected_2D_x_clip = torch.clamp(pts2d_pro[:, 0:1], 0, clamp_to_resolution[0])
        coords_projected_2D_y_clip = torch.clamp(pts2d_pro[:, 1:2], 0, clamp_to_resolution[1])
        pts2d_pro = torch.cat([coords_projected_2D_x_clip, coords_projected_2D_y_clip], dim=-1)
        pts2d_pro = pts2d_pro.reshape((batch_size, -1, 2))
